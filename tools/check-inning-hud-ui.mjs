...
try {
  ...
  for(const [width,height] of [[320,568],[390,844],[844,390],[1440,1000]]) {
    ...
    for(const cls of ['.inning-sound','.inning-exit','.inning-look','.batting-hold','.inning-throw']){const b=await page.locator(cls).boundingBox();assert.ok(b.height>=44&&b.x>=0&&b.x+b.width<=width&&b.y+b.height<=height);}
    ...
    await page.locator('.is-deciding .batting-decision').waitFor({timeout:60000});
