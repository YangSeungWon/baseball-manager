...
try {
 ...
 for(const [speed,expected,width,height] of [[5.5,'1B',390,844],[9,'2B',844,390]]){
  ...
  await page.evaluate(async speed=>{
   localStorage.setItem('dugout.sfx','0');const {BattingGame}=await import('/js/batting-game.js');const resolve=BattingGame.prototype.resolvePitch;
   BattingGame.prototype.resolvePitch=function(c){
    this.bases=[false,false,false];this.baseRunners=[null,null,null];Object.defineProperty(this,'batter',{configurable:true,get:()=>({id:'test-runner',name:'테스트 타자',speed,style:'공격형'})});
    const e=resolve.call(this,c,[0,.1,.1,.9,1-.5/.85,6/43,.5,.75,.5,.5]);window.result=e;return e;
   };
  ...
  await page.locator('#btnBatting').click();await page.locator('.inning-throw').click();await page.locator('.is-deciding').waitFor();await page.locator('.batting-swing').click();
