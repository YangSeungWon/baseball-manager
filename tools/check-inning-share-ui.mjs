...
try {
  ...
  await page.goto(url+'/?challenge=b4-42');
  ...
  assert.match(await page.evaluate(()=>shared.url),/challenge=b4-42$/);
  assert.match(await page.evaluate(()=>shared.text),/1구 · 3득점 · 1안타/);
  await page.locator('[data-copy]').click();assert.match(await page.evaluate(()=>copied),/challenge=b4-42$/);
