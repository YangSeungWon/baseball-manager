...
test('challenge links preserve uint32 seeds and reject malformed or unsupported versions',()=>{
  for(const seed of [0,42,4294967295])assert.equal(challengeSeed('?challenge=b3-'+seed),seed);
  for(const q of ['', '?challenge=b3--1','?challenge=b3-4294967296','?challenge=b3-1x','?challenge=b1-42','?challenge=b3-01'])assert.equal(challengeSeed(q),null);
});
test('shared results use complete run records and reproduce the same starting challenge',()=>{
  ...
  assert.match(result.text,new RegExp(a.count+'구'));assert.ok(result.url.endsWith('b3-7'));
