...
export function playFullGame(policy,seed,{sigma=.05}={}){
  const g=new FullGame(seed),t=tally(),r=lcg(seed*2654435761+3);policy.reset?.(seed);
  while(!g.done){
    if(g.half==='top'){
...
if(process.argv[1]&&process.argv[1].endsWith('measure-batting.mjs')){
  ...
  console.log('\n사람 손 (스테이지1) — 오차를 가진 정책. 목표는 구간이 아니라 하한이다.');
  const H=hands(Math.max(200,Math.round(n/4)));
  for(const [k,m] of Object.entries(H)){
    const floor=HAND_FLOOR[k];
    console.log(`\n[${HANDS[k].label}]`);
    row('컨택률 (%)',m.contact,floor.contact?`≥ ${floor.contact}`:'');
    row('헛스윙률 (%)',m.whiff,'');row('파울률 (%)',m.foul,'');
    row('타구 안타율 (%)',m.hit,floor.hit?`≥ ${floor.hit}`:'');
    row('존 밖 스윙률 (%)',m.chase,'');row('스테이지1 클리어 (%)',m.clear,'');
  }
