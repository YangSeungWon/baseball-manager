...
- 투구를 시작하면 3D가 화면 전체를 채우고 조작부가 숨겨집니다. 점수와 판정
  위치는 남으며, 결과를 확인한 뒤 선택 화면으로 돌아옵니다. 브라우저의
  전체 화면 권한은 요청하지 않습니다.
- 선택 요약, B/S/O 표시, 주자 다이아몬드, 남은 목표와 최근 5구를 제공합니다.
  모바일 실행 버튼은 하단에 유지하고, PC는 구장과 조작부를 나란히 배치합니다.
