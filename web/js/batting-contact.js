...
export function battingContact({aim,pitch,timing,power=.5,batter={}}){
  ...
  const label=kind==='miss'?(reason==='timing'?(seconds<0?'배트가 먼저 지나감':'공이 먼저 지나감'):reason==='aim'?'배트가 코스를 벗어남':'배트 끝에 닿지 않음'):kind==='foul'?(tipped?'깎여 맞은 파울':reason==='barrel'?(Math.sign(dxm)===hand?'배트 끝에 맞은 파울':'손잡이 쪽에 맞은 파울'):'비껴 맞은 파울'):kind==='solid'?'정타':'빗맞음';
  // 파울 방향은 타자에게 주는 정보다. 타석 쪽으로 감기면 당겨 친 것(빠름), 반대쪽으로 흐르면 밀린 것(늦음).
  // gap 은 공을 기준으로 배트 중심이 어디에 있었는지(m). 화면이 컨택 포인트를 그릴 때 쓴다.
  return {kind,reason,label,quality,distance,seconds,speed:tipped?speed*C.tipSpeed:speed,launch,angle,hand,timingAngle,tipped,
