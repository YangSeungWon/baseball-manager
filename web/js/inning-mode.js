...
import { pitchPressure, releaseMarker, pitchTarget } from './pitch-control.js';
...
import { battingFlightSeconds, battingPressTiming, battingCatchSeconds, battingCheckDepth, battingSwing } from './batting-input.js';
...
export function openInningMode(role='pitcher',initialSeed=null,initialStage=0,continuedGame=null) {
  ...
  if(batting)root.insertAdjacentHTML('beforeend','<div class="batting-plan" role="group" aria-label="타격 전략">'
    +'<div class="plan-guess" data-group="target">'+[['any','노림 없음'],['FF','직구'],['SL','슬라이더'],['CH','체인지업']]
  ...
  let batAim={x:0,z:0},cutNext=false;
  let seed=Number.isInteger(initialSeed)&&initialSeed>=0&&initialSeed<=0xffffffff?initialSeed:crypto.getRandomValues(new Uint32Array(1))[0],game,lv,busy=false,dead=false,events=[],runEvents=[],shown=null,scrollBefore=0,choice=batting?{target:'any',approach:'contact',location:'any',action:'swing'}:{type:'FF',zone:'out',intent:'attack'};   // approach/action stay for the zone overlay; the swing starts on press
  let sound=true;try{sound=localStorage.getItem('dugout.sfx')!=='0';}catch{}
  let ambience='idle',intensity=.5,cancelDecision=null,releaseNow=null,calm=false,maxEffort=false;
  ...
  // 타격 조작은 입장 화면에서 안내한다. 경기 중 코치는 투수 편에만 사용한다.
  const COACH_KEY='dugout.coach.v1';let coachSeen={};try{coachSeen=JSON.parse(localStorage.getItem(COACH_KEY)||'{}');}catch{}
  const wantsCoach=new URLSearchParams(location.search).has('coach');
  const coachRole=full?'batter':batting?'batter':'pitcher';
  let coaching=!batting&&(wantsCoach||!coachSeen[coachRole])&&!continuedGame,coachStep=null;
  const coachEl=document.createElement('div');coachEl.className='coach-tip';coachEl.hidden=true;coachEl.innerHTML='<p></p><button type="button" class="coach-skip">건너뛰기</button>';root.append(coachEl);
  coachEl.querySelector('.coach-skip').onclick=()=>coachDone();
  function coachSay(step,text,{pulse=false,at=null}={}){
    if(!coaching)return;coachStep=step;coachEl.hidden=false;coachEl.querySelector('p').innerHTML=text;coachEl.classList.toggle('is-pulse',pulse);
    coachEl.classList.remove('at-hold','at-picks','at-pitch');if(at)coachEl.classList.add('at-'+at);
  }
  function coachAfter(){
    if(coachStep==='pitch')coachSay('effort','<b>숨 고르기</b>는 제구를, <b>전력</b>은 구속을 줍니다. 둘 다 한 공짜리입니다.',{at:'pitch'});else coachDone();
  }
  function coachHide(){coachEl.hidden=true;coachStep=null;coachEl.classList.remove('is-pulse');}
  function coachDone(){if(!coaching)return;coaching=false;coachHide();coachSeen[coachRole]=true;try{localStorage.setItem(COACH_KEY,JSON.stringify(coachSeen));}catch{}}
  ...
  // 타임: 화면에 띠가 뜨고 투수는 세트를 푼다. 화면이나 Space를 누르면 경기를 재개한다.
  ...
  const timeBanner=document.createElement('div');timeBanner.className='time-banner';timeBanner.innerHTML='<b>TIME</b><span>화면을 누르거나 Space를 누르면 계속합니다</span>';root.append(timeBanner);
  if(!batting){$('.pitch-breathe').onclick=()=>breathe();$('.pitch-effort').onclick=()=>{if(busy||calm||game.done)return;maxEffort=!maxEffort;$('.pitch-effort').setAttribute('aria-pressed',String(maxEffort));root.classList.toggle('is-max-effort',maxEffort);selection();};}
  function dock(){
    if(batting){root.classList.toggle('is-paused',paused);root.classList.toggle('is-timeout',paused);plan2();}
  }
  // 커트는 2스트라이크에서만 고를 수 있다. 카운트가 풀리면 선택도 풀린다.
  function plan2(){
  ...
  async function breathe(){
    if(busy||calm||dead||game.done)return;maxEffort=false;$('.pitch-effort').setAttribute('aria-pressed','false');root.classList.remove('is-max-effort');
  ...
  async function releasePitch(startAt=performance.now()){
    ...
    meter.querySelector('span').textContent=calm?'안정':maxEffort?'전력':pressure>.65?'긴장':pressure>.4?'집중':'여유';$('.pitch-effort').disabled=true;
    return new Promise(resolve=>{
      ...
      const tick=()=>{const p=Math.min(1,(performance.now()-begin)/duration),v=releaseMarker(p,pressure,effortMode());displayed=v;$('.pitch-needle').style.left=((v+1)*50)+'%';lv.S.pitcherWind=Math.min(1,p*2);if(p>=1)finish(1);else raf=requestAnimationFrame(tick);};
  ...
  async function readPitch(pitch){
    ...
    const picked=await new Promise(resolve=>{
      ...
      // 준비한 채 공이 지나가면 배트가 끝까지 나온 체크 스윙이다.
      const expire=()=>{preview.step(to);if(loadedAt!==null)finish('take',{check:{depth:1}});else finish('take');};
      const hold=()=>{if(settled||loadedAt===null)return;
        if(!open){unload();return;}
        const now=performance.now();if(now>=end){expire();return;}
        preview.step(pitchTime(now));const depth=battingCheckDepth(preview.t,arrival);
        if(depth<=0){unload();return;}
        lv.S.batSwingFrom=.25;lv.S.swing=.25+BATTING.playerInput.check.peak*depth;
        finish('take',{check:{depth}});};
      ...
      const setAim=(x,z)=>{aim={x:Math.max(-BATTING_AIM_LIMIT,Math.min(BATTING_AIM_LIMIT,x)),z:Math.max(-BATTING_AIM_LIMIT,Math.min(BATTING_AIM_LIMIT,z))};batAim=aim;lv.S.aim=aim;paintAim();};
      const press=()=>{if(settled||loadedAt===null)return;
        if(!open){unload();return;}   // 공을 놓기 전에 뗐다면 준비만 푼다
        const now=performance.now();if(now>=end){expire();return;}
        const swing=battingSwing(swingKind());
        preview.step(pitchTime(now));timing=battingPressTiming(preview.t,arrival,swing.seconds);lv.S.batSwingFrom=.25;
        finish('swing',{power:swing.power,aim:{...aim}});};
      const surface=lv.three?.canvas||null;
      if(surface)mountBattingPointer(surface,{signal:input.signal,aimAt:(x,y)=>lv.three.battingAimAt(x,y),onAim:setAim,onLoad:load,onSwing:press,onCheck:hold,onDrop:unload,onPreview:p=>{
        lv.S.pointerAiming=!!p;pointer.hidden=!p;if(p?.touch)cancelArea.hidden=false;
        if(!p)return;
        ...
        const label=p.cancel?'놓으면 참기':'여기서 떼면 스윙';if(cancelArea.textContent!==label)cancelArea.textContent=label;
      }});
      setAim(batAim.x,batAim.z);
      (async()=>{
        await runTimeline(warm);if(dead||settled)return;
        lv.S.battingDecision=true;root.classList.add('is-deciding');dock('read');
        readZone.hidden=true;readZone.classList.add('is-reading-pitch');paintRead(0);
        $('.inning-feedback').textContent='';
        begin=performance.now();end=begin+readMs;open=true;
        // 터치에서는 스윙 패드가 어디인지 손을 대기 전에 보여야 한다.
        if(matchMedia('(pointer:coarse)').matches){
          // 보이는 띠와 실제로 눌리는 영역의 높이를 맞춘다(batting-pointer 의 padOf 와 같은 식).
          const r=lv.three?.canvas?.getBoundingClientRect();
          if(r)cancelArea.style.height=Math.min(120,Math.max(72,r.height*.18))+'px';
          cancelArea.textContent='여기서 떼면 스윙';cancelArea.hidden=false;
        }
        const tick=()=>{const now=performance.now(),left=Math.max(0,end-now);preview.step(Math.min(to,pitchTime(now)));paintRead(Math.max(0,Math.min(1,(preview.t-release)/flight)));
          if(!left)expire();else raf=requestAnimationFrame(tick);};
        timer=setTimeout(expire,readMs);tick();surface?.focus({preventScroll:true});
      })();
    });
    return picked;
  }
  ...
  function zone(event=null) {
    ...
    const pressure=pitchPressure(game.snapshot()),spread=(10+pressure*9)*(calm?.68:maxEffort?1.35:1);
    ...
    const aimDot=batting?`<circle class="zone-aim" cx="100" cy="96" r="7"/>`:'';
    root.querySelectorAll('.zone-markers').forEach(g=>g.innerHTML=coverage+pitchingRisk+target+leaders.join('')+marks+aimDot);
  ...
  // 투수 편: 고른 코스와 승부 방식대로 포수가 미트를 내민다. 조준의 시각적 목표다.
  ...
  function history() {
    $('.inning-history').innerHTML=events.length?'<span>현재 타석 · 최근 투구</span>'+events.slice(-5).map(e=>`<span class="inning-pitch-chip"><small>${e.pitchNumber}구 · ${PITCHES[e.pitch.t].name}</small><b>${e.label}</b></span>`).join(''):(batting?'<span>현재 타자 · 첫 공을 기다립니다.</span>':'<span>현재 타자 · 첫 공을 고르세요.</span>');
  }
  ...
  async function intro(){
    ...
    if(!batting){$('.pitch-breathe').disabled=true;$('.pitch-effort').disabled=true;}
    ...
    if(batting||full)card.insertAdjacentHTML('beforeend','<div class="match-batting-help" aria-label="타격 조작"><div class="batting-help-mouse"><p><b>조준</b> 마우스 이동 — 배트는 마지막 자리에 남습니다</p><p>왼쪽 버튼을 누르면 준비, 떼면 <b>스윙</b></p><p>투수가 공을 놓기 전부터 누르고 있으면 <b>장타</b>, 공을 보고 누르면 <b>컨택</b></p><p>누른 채 오른쪽 클릭하면 참습니다. 늦게 참으면 심판이 체크 스윙을 판정합니다.</p></div><div class="batting-help-touch"><p>화면 위쪽을 끌어 <b>조준</b> — 손을 떼도 배트는 그 자리에 남습니다</p><p>아래 <b>스윙 패드</b>를 누르면 준비, 떼면 <b>스윙</b></p><p>투수가 공을 놓기 전부터 누르고 있으면 <b>장타</b>, 공을 보고 누르면 <b>컨택</b></p><p>패드 밖으로 밀어 올린 뒤 떼면 참습니다. 늦게 참으면 심판이 체크 스윙을 판정합니다.</p></div></div>');
    ...
    else if(coaching)coachSay('pitch','<b>버튼이나 화면을 누르고 있으면</b> 와인드업, <b>놓는 순간</b> 던집니다. 바늘이 가운데일 때 놓으세요.<kbd>Space</kbd>',{at:'pitch'});
    if(batting)dock();else{$('.pitch-breathe').disabled=false;$('.pitch-effort').disabled=false;}
  }
  ...
  function finish() {
    root.classList.add('is-finished');coachHide();
  ...
  const play=async(action='swing',{pressAt=null}={})=>{
    ...
    try {
      let e,resumeAt=0;
      if(batting){const pitch=game.preparePitch(choice),cut=cutNext&&game.strikes>=2,picked=await readPitch(pitch);if(dead||!picked?.action)return;resumeAt=picked.time;e=game.decidePitch(picked.action,picked.timing,picked.power,picked.aim,picked.check,cut);}
      else {const release=await releasePitch(pressAt??performance.now());if(dead||release===null)return;e=game.pitch({...choice,release,effort:effortMode()});resumeAt=1.55;calm=false;maxEffort=false;$('.pitch-effort').setAttribute('aria-pressed','false');root.classList.remove('is-max-effort');}
      ...
      const arrival=lv._pitch(tl,{...e.pitch,r,...(e.impact?{contactQuality:e.impact.quality,foulAngle:e.impact.angle,foulLaunch:e.impact.launch}:{}),...(batting?{flightSeconds:battingFlightSeconds(e.pitch.v)}:{}),...(batting&&e.choice.action==='swing'?{swingStart:resumeAt,batSeconds:battingSwing(e.choice.approach).seconds}:{}),...(e.check?{checkAt:resumeAt,checkPeak:.25+BATTING.playerInput.check.peak*e.check.depth}:{}),swingLead:e.timing?.p!=null?(e.timing.window.from+e.timing.window.to)/2-e.timing.p:0},0,.65,rec,{last:true});
      if(!batting)tl.at(Math.max(0,arrival-.35),()=>{if(S.fielders.C)S.fielders.C.catchTarget=null;});
      ...
      tl.at(arrival,()=>{if(r==='X'){atmosphere('contact');$('.inning-feedback').textContent=e.impact?.label||'타격';}else react();});
      ...
      if(r==='X') {
        ...
        tl.at(arrival,()=>{S.hold=null;if(!batting)S.batter=null;S.broadcast={kind:batting?'pitch':'field'};S.fieldPlay={physical:true,phase:'flight',fielder:play.handler};});
        if(batting)tl.at(arrival+.18,()=>{S.batter=null;S.broadcast={kind:homeRun?'bat-flip':'field'};});
        tl.add(arrival,play.duration,k=>{
          ...
          if(frame.runners)S.runners=frame.runners.filter(r=>r.vis&&(!batting||k*play.duration>=.18||r.id!==e.before.batter.id)).map(r=>({...r,alpha:1}));
          if(batting&&homeRun){
